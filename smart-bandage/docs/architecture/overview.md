...
| `digital_twin/hidden_state.py` | Real (DT-1) — a per-device `DeviceState` vector (bacterial_load, inflammation, healing_stage_progress, moisture), Euler-integrated one tick at a time instead of read off a scenario's checkpoint array. Only `bacterial_load` has real dynamics (logistic growth) plus `infection_biomarker()` turning that hidden load into the observable biomarker; the other three fields hold their initial value. Standalone — not wired into `simulator/`, `processing/`, or any later DT ticket (DT-2 built its own, unrelated shared-state model instead; see its row below). `tests/test_digital_twin_dt1.py` |
| `digital_twin/state.py`, `digital_twin/observation.py`, `digital_twin/device_physics.py` | Real (DT-2) — `WoundState`: one shared, mean-reverting latent state (inflammation, bacterial_load, moisture, perfusion) per device, advanced by wall-clock `dt_seconds` instead of a tick index, read (never scripted) by every channel's transfer function in `observation.py` so multiple channels move together off one underlying cause. `device_physics.py` runs battery drain, electrode fouling and BLE link quality as the same kind of continuous, state-coupled process. `DigitalTwinDevice` (`observation.py`) is what `backend/app/simulation.py` (DT-6) seeds per device. Originally collided with DT-1 on the `state.py` path; DT-1 moved to `hidden_state.py` to resolve it. `tests/test_digital_twin_dt2.py` |
| `digital_twin/engine.py`, `digital_twin/adapter.py` | Real (DT-3) — `DigitalTwinEngine`: the same 7 scenario configs and signal generators `ScenarioSensor` uses, replayed on a tick counter instead of `time.monotonic()`, with every random draw seeded (`random.Random`) instead of drawn from the global `random` module. `engine.run(n)` generates hours or days of trajectory in one call with no sleeping, and the same seed reproduces a run bit-for-bit. `DigitalTwinSensor` (`digital_twin/adapter.py`) wraps it behind `SensorInterface` so it's an opt-in backend inside `MultiChannelSensor` (`engine="digital_twin"`) rather than a replacement for `ScenarioSensor` — the default stays unchanged. `tests/test_digital_twin_dt3.py` |
| `digital_twin/profile_state.py`, `digital_twin/perturbations.py`, `digital_twin/profiles.py` | Real (DT-4) — the 7 Phase 2 scenarios re-expressed as parameterized `Perturbation`s (`InfectionOnset`, `DressingDisturbance`, `ElectrodeFouling`) against a day-paced `WoundState` (its own model, distinct from DT-2's — see `profile_state.py`'s docstring for why), packaged into three named clinical presets (`normal_healing`, `complicated_infection`, `chronic_wound`) `profiles.simulate()` steps forward tick by tick. Standalone — not yet wired into a sensor or the backend (DT-6 built its own small, self-contained profile dict instead of depending on this while it was still settling; migrating it to this registry is future work). `tests/test_digital_twin_profiles.py` |
| `backend/app/simulation.py` (`DigitalTwinDevice` path), `backend/app/routers/simulation.py`, `frontend/src/components/TwinControlPanel.tsx` | Real (DT-6) — a twin-backed simulation mode alongside the Phase 4 scenario-backed one, behind the same three endpoints: `POST /simulation/start` optionally names a patient profile, seeding a DT-2 `DigitalTwinDevice` in place of the usual `MultiChannelSensor`; `GET /simulation/patient-profiles` and `GET /simulation/twin/{device_id}` (dev-only ground-truth overlay, gated off in production) back the dashboard's `TwinControlPanel`. Everything downstream of one raw reading per channel per tick — pipelines, alerts, WS broadcast shape — is unchanged; a caller that never passes `patient_profile` gets the exact scenario-backed behavior back. `/simulation/start` now rejects an empty `channels` list and a `twin_channel_id` outside `channels` with a 400 instead of, respectively, an unhandled `IndexError` and a request that silently never produces a ground-truth snapshot. `backend/tests/test_api.py` |
