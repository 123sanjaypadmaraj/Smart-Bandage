...
| `digital_twin/observation.py` (`ChannelProfile.unit`/`cal_slope`/`cal_intercept`), `backend/app/simulation.py` (`_twin_pipeline`, `_assign_channel_sensor_types`) | Real (sim-realism) — twin-backed channels previously ran through an identity calibration with `unit="a.u."`, so a "pH" channel's dashboard reading was a bare number like `104.3` with no unit — not something a real bandage could report. Each known `sensor_type` (`pathogen_channel_1`, `glucose`, `pH`) now carries its own linear calibration curve mapping `raw_signal` to a literature-plausible range and real unit (CRP-like `mg/L`, wound-fluid glucose `mg/dL`, wound-bed `pH`) — see `ChannelProfile`'s docstring for the reasoning and caveats (approximated ranges, not fit to real electrode data, same honesty as `processing/calibration/experimental.py`'s synthetic ground truth). A multi-channel twin-backed device also no longer clones one `sensor_type` across every channel; `_assign_channel_sensor_types` keeps the patient profile's own sensor as the first (ground-truth-overlay) channel and rotates the rest through the other known types, so a 3-channel device reads as pathogen + pH + glucose. The scenario-backed (non-twin) path is unchanged — still identity/`a.u.` — this only applies to `patient_profile`-driven runs. `tests/test_digital_twin_dt2.py`, `backend/tests/test_api.py::test_twin_backed_simulation_reports_real_units_not_arbitrary_units` |
