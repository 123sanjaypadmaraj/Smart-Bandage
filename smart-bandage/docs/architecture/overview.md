...
| `digital_twin/state.py` | Real, in progress (DT-1/DT-7) — a per-device `DeviceState` vector (bacterial_load, inflammation, healing_stage_progress, moisture), Euler-integrated one tick at a time instead of read off a scenario's checkpoint array. Only `bacterial_load` has real dynamics so far (logistic growth) plus `infection_biomarker()` turning that hidden load into the observable biomarker — proven on this one channel end-to-end before the other three get their own dynamics. Not yet wired into `simulator/` or `processing/`. See `docs/architecture/digital_twin.md` for the design and testing strategy (property-based bounds + a seeded regression trajectory). `tests/test_digital_twin_dt1.py`, `tests/test_digital_twin_dt7_properties.py` |
