...
| — Containerization | Docker images + compose for backend/dashboard | ✅ done — `docker/`, `docker-compose.yml`; Postgres is now the default (`docker compose up --build`), matching what `ENVIRONMENT=production` actually requires — the old zero-dependency SQLite path moved to the standalone `docker-compose.sqlite.yml`. `docker-compose.tls.yml` overlay adds real HTTPS (nginx + uvicorn's built-in TLS) — config-validated (`docker compose config`), not yet run end to end (no reachable Docker daemon in the environment that wrote it) |
